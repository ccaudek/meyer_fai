...
new <- data.frame(
  id = mydata$id,
  sick = variables$`ID 1`,
  n_hospitalization = variables$"NUMERO RICOVERI",
  n_er = variables$"NUMERO ACCESSI PS",
  f3s
)
...
new1 <- new[new$n_hospitalization != 0, ]
cor(new1$n_hospitalization, new1$f1s, use = "complete.obs")
plot(jitter(new1$n_hospitalization), jitter(new1$f1s))


new1 <- new[new$n_er != 0, ]
cor(new1$n_er, new1$f1s, use = "complete.obs")
plot(jitter(new1$n_er), jitter(new1$f1s))
