...
selected_items <- c(
  "i_86", "i_57", "i_5", "i_85", "i_81", "i_105", "i_48", "i_133", "i_129", 
  "i_39", "i_103", "i_143", "i_79", "i_111", "i_34", "i_119", "i_116", 
  "i_23", "i_45", "i_41", "i_186", "i_38", "i_128", "i_7", "i_16", "i_29", 
  "i_137", "i_96", "i_194"
)
...
esem_model <- '
  # Factor 1: child_characteristics
  child_characteristics =~ item_86 + item_57 + item_5 + item_85 + item_81

  # Factor 2: caregiving_requests
  caregiving_requests =~ item_105 + item_48 + item_133 + item_129 + item_39 + item_103 + item_79

  # Factor 3: perception_cure
  perception_cure =~ item_143 + item_79

  # Factor 4: intrapsychic_factors
  intrapsychic_factors =~ item_111 + item_34 + item_119 + item_116 + item_23 + item_45 + item_41

  # Factor 5: coping
  coping =~ item_186 + item_128 + item_7 + item_16 + item_29

  # Factor 6: hyper_protection
  hyper_protection =~ item_137 + item_96 + item_194
'

# Fit the ESEM model
esem_fit <- sem(esem_model, data = imp_selected, ordered= items)

# Summarize the results
summary(esem_fit, standardized = TRUE, fit.measures = TRUE)

# Define the item names
items <- c(
  "item_86", "item_57", "item_5", "item_85", "item_81",  # Factor 1
  "item_105", "item_48", "item_133", "item_129", "item_39", "item_103", "item_79",  # Factor 2
  "item_143", "item_79",  # Factor 3
  "item_111", "item_34", "item_119", "item_116", "item_23", "item_45", "item_41",  # Factor 4
  "item_186", "item_128", "item_7", "item_16", "item_29",  # Factor 5
  "item_137", "item_96", "item_194"  # Factor 6
)

# Define the ESEM model with cross-loadings using target rotation
esem_model <- "
  F1 =~ item_86 + item_57 + item_5 + item_85 + item_81
  F2 =~ item_105 + item_48 + item_133 + item_129 + item_39 + item_103 + item_79
  F3 =~ item_143 + item_79
  F4 =~ item_111 + item_34 + item_119 + item_116 + item_23 + item_45 + item_41
  F5 =~ item_186 + item_128 + item_7 + item_16 + item_29
  F6 =~ item_137 + item_96 + item_194
"

# Fit the ESEM model with target rotation and treating items as ordinal
fit_esem <- efaUnrotate(
  esem_model, data = imp_selected, nfactors = 6, ordered = TRUE, rotation = "target"
  )
...
bifactor_model <- "
  General =~ item_86 + item_57 + item_5 + item_85 + item_81 +
              item_105 + item_48 + item_133 + item_129 + item_39 + item_103 + item_79 +
              item_143 + item_79 +
              item_111 + item_34 + item_119 + item_116 + item_23 + item_45 + item_41 +
              item_186 + item_128 + item_7 + item_16 + item_29 +
...
# Fit the bifactor model treating items as ordinal
fit_bifactor <- cfa(
  bifactor_model, 
  data = imp_selected, 
  ordered = TRUE, 
...
items <- c(
  "item_86", "item_57", "item_5", "item_85", "item_81",  # Factor 1
  "item_105", "item_48", "item_133", "item_129", "item_39", "item_103",  # Factor 2 (Removed item_79)
  "item_111", "item_34", "item_119", "item_116", "item_23", "item_45",  # Factor 4 (Removed item_41)
  "item_186", "item_128", "item_7", "item_29",  # Factor 5 (Removed item_16)
  "item_137", "item_96"  # Factor 6 (Removed item_194)
)
...
bifactor_model <- "
  General =~ item_86 + item_57 + item_5 + item_85 + item_81 +
              ...
              item_111 + item_34 + item_116 + item_23 + item_45 +
              item_186 + item_128 + item_7 + item_29 +
              item_137 + item_96

  F1 =~ item_86 + item_57 + item_5 + item_85 + item_81
  F2 =~ item_105 + item_48 + item_133 + item_129 + item_39 + item_103
  F4 =~ item_111 + item_34 + item_116 + item_23 + item_45
  F5 =~ item_186 + item_128 + item_7 + item_29
  F6 =~ item_137 + item_96
"
...
# Define the refined bifactor model
bifactor_model <- "
  General =~ item_86 + item_57 + item_5 + item_85 + item_81 +
              item_105 + item_48 + item_133 + item_129 + item_39 + item_103 +
              item_116 + item_23 + item_45 +
              item_186 + item_7 + item_29 +
              item_137 + item_96

  F1 =~ item_86 + item_57 + item_5 + item_85 + item_81
  F2 =~ item_105 + item_48 + item_133 + item_129 + item_39 + item_103
  F4 =~ item_116 + item_23 + item_45
  F5 =~ item_186 + item_7 + item_29
  F6 =~ item_137 + item_96
"
...
# Define the base model items
base_items <- c(
  "item_86", "item_57", "item_5", "item_85", "item_81",  # Factor 1
  "item_105", "item_48", "item_133", "item_129", "item_39", "item_103",  # Factor 2
  "item_116", "item_23", "item_45",  # Factor 4
  "item_186", "item_7", "item_29",  # Factor 5
  "item_137", "item_96"  # Factor 6
)

# Load dataset (Assuming 'data' contains all items and death_risk variable)
# data <- read.csv("your_data.csv")
imp_selected$death_risk <- df$death_risk

# Extract available items and target variable
target_var <- "death_risk"
candidate_items <- setdiff(names(df[, 1:182]), c(base_items, target_var))

# Select best 10 additional items using logistic regression
ctrl <- rfeControl(functions = rfFuncs, method = "cv", number = 5)
fit <- rfe(df[, candidate_items], df[[target_var]], sizes = c(1:10), rfeControl = ctrl)
best_items <- predictors(fit)
best_items
# [1] "item_159" "item_60"  "item_154" "item_157" "item_54"  "item_106" "item_40" 
# [8] "item_20"  "item_139" "item_125" 


mod_6f <- "
  F1 =~ item_86 + item_5 + item_81 + item_60
  F2 =~ item_105 + item_48 + item_133 + item_129 + item_39 + item_103 
  F4 =~ item_116 + item_23 + item_20 
  F5 =~ item_29 + item_157 
  F6 =~ item_137 + item_96 + item_159
"
...
indicator_names <- lavNames(fit_6f, type = "ov")
...
model <- '
    ...
    efa("efa1")*f1 + 
    efa("efa1")*f2 + 
    efa("efa1")*f3 =~ item_86 + item_5 + item_81 + item_60 +
      item_105 + item_48 + item_133 + item_129 + item_39 + item_103 +
      item_116 + item_23 + item_20 + 
      item_29 + item_157 +
      item_137 + item_96 + item_159
'
...
selected_items <- c(
  "item_86", "item_5", "item_81", "item_60",
  "item_105", "item_48", "item_133", "item_129", "item_39", "item_103",
  "item_116", "item_23", "item_20", 
  "item_29", "item_157",
  "item_137", "item_96", "item_159"
...
df1 <- df %>% 
  dplyr::select(all_of(selected_items))


# Define the target matrix with 18 rows and 5 columns
TARGET <- matrix(0, nrow = 18, ncol = 5)
...
## Best subsect selection for classification accuracy ---------------------
library(caret)
library(glmnet)
library(pROC)
library(dplyr)

# Convert death_risk to a factor if not already
df$death_risk <- as.factor(df$death_risk)

# Define predictor matrix and response variable
X <- as.matrix(df[, 1:182])  # Convert to matrix for glmnet
y <- df$death_risk  # Binary outcome
...
selected_features <- which(coef(lasso_model)[-1] != 0)
best_predictors <- colnames(X)[selected_features]
cat("Selected Best Predictors:", best_predictors, "\n")
#' Selected Best Predictors: item_7 item_9 item_11 item_14 item_19 item_20 
#' item_22 item_23 item_24 item_26 item_34 item_40 item_41 item_44 item_45 
#' item_52 item_54 item_60 item_61 item_64 item_66 item_69 item_76 item_85 
#' item_90 item_94 item_99 item_101 item_121 item_122 item_125 item_133 
#' item_134 item_135 item_139 item_142 item_153 item_154 item_156 item_157 
#' item_159 item_164 item_168 item_180 item_185 item_187 item_194 

# Train logistic regression using selected predictors
final_train_data <- df[trainIndex, c(best_predictors, "death_risk")]
final_test_data <- df[-trainIndex, c(best_predictors, "death_risk")]

logit_model <- glm(death_risk ~ ., data = final_train_data, family = binomial)

# Predict on test set
pred_probs <- predict(logit_model, final_test_data, type = "response")
pred_classes <- ifelse(pred_probs > 0.5, 1, 0)

# Compute classification accuracy
accuracy <- mean(pred_classes == final_test_data$death_risk)
cat("Classification Accuracy with Best Subset:", accuracy, "\n")

# Compute AUC
roc_curve <- roc(final_test_data$death_risk, pred_probs)
auc_value <- auc(roc_curve)
cat("AUC with Best Subset:", auc_value, "\n")

library(stabs)

stability_model <- stabsel(
  x = as.matrix(df[, 1:182]), 
  y = df$death_risk, 
  fitfun = glmnet.lasso, 
  q = 40,  # Numero massimo di item da selezionare
  # PFER = .9  # Controlla il tasso di falsi positivi
  cutoff = 0.75  # Un item deve apparire nel 75% dei subsample per essere selezionato
)

selected_items <- stability_model$selected
selected_items
...
# # Bifactor ESEM -----------------------------------------------------------
# 
# # Define the target matrix with 18 rows and 6 columns
# TARGET <- matrix(0, nrow = 18, ncol = 6)
# 
# # General factor (g): All items load on the general factor
# TARGET[, 1] <- 1
# 
...
# model_bifactor <- '
#     # General factor (g)
#     g =~ NA*item_86 + item_5 + item_81 + item_60 +
#          item_105 + item_48 + item_133 + item_129 + item_39 + item_103 +
#          item_116 + item_23 + item_20 + 
#          item_29 + item_157 +
#          item_137 + item_96 + item_159
#     g ~~ 1*g  # Fix variance of g to 1
# 
#     # Specific group factors using single ESEM block
#     efa("efa1")*F =~ item_86 + item_5 + item_81 + item_60 +
#                      item_105 + item_48 + item_133 + item_129 + item_39 + item_103 +
#                      item_116 + item_23 + item_20 + 
#                      item_29 + item_157 +
#                      item_137 + item_96 + item_159
